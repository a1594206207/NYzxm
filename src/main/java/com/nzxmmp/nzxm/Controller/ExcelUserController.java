...
import com.nzxmmp.nzxm.Service.ServiceImpl.OutSelectServiceImpl;
...
public class ExcelUserController {
    ...
    @Autowired
    private ExcelInsert excelInsert;
    ...
    public void downloadTemplate(HttpServletResponse response, HttpServletRequest request) throws Exception {
        ...
        if (state.equals("天津高新区内资半年有望项目明细表导入模板")) {
             workbook = excelServiceB.outTemplate();
            response.setContentType("application/vnd.ms-excel");
            response.setHeader("content-Disposition", "attachment;filename=" + URLEncoder.encode("天津高新区内资半年有望项目明细表导入模板.xlsx", "UTF8"));

        } else if (state.equals("天津高新区内资签约落地项目明细表导入模板")) {
            workbook = excelServiceQ.outTemplate();
            response.setContentType("application/vnd.ms-excel");
            response.setHeader("content-Disposition", "attachment;filename=" + URLEncoder.encode("天津高新区内资签约落地项目明细表导入模板.xlsx", "UTF8"));
    ...
    public void outNz(OutSelectReq outSelectReq,HttpServletResponse response) throws IOException {
        ...
        //导入方法
        outSelectService.outputNzxm(outputStream,outSelectReq);
