...
import com.nzxmmp.nzxm.entity.YzQyldExcel;
 ...
 *
 */
@Service("YzxmExcelServiceQyldImpl")
public class YzxmExcelServiceQyldImpl implements ExcelServic {
    ...
    public Workbook outTemplate() {
        ClassPathResource classPathResource = new ClassPathResource("ExcxelTemplate/外资签约落地模板.xlsx");
    ...
    public String excelAddMysql(MultipartFile file) {
        ...
        try {
            ...
            List<YzQyldExcel> list = new ArrayList<>();

            try {
//

                list= EasyExcel.read(file.getInputStream(), YzQyldExcel.class,new EventListener()).sheet().headRowNumber(4).doReadSync();
            ...
            for (int i = 0; i < list.size(); i++) {
                YzQyldExcel excelModel = list.get(i);

                Class<?> clazz = YzQyldExcel.class;  //得到类对象！
            ...
            for (YzQyldExcel excelQyld : list) {
                Xmmcs.add(excelQyld.getXmmc());
            }
            ...
            for (YzQyldExcel excelQyld : list) {
                Yzxmbd one = yzxmdao.selectToName(excelQyld.getXmmc());
                if(one!=null){
                    stringXmmc.add(excelQyld.getXmmc());
                }
            }
